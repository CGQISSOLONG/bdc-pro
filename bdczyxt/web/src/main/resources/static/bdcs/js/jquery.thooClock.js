(function($) {
	$.fn.thooClock = function(options) {
		this.each(function() {
			var cnv, ctx, el, defaults, settings, radius, dialColor, dialBackgroundColor, secondHandColor, minuteHandColor, hourHandColor, alarmHandColor, alarmHandTipColor, hourCorrection, x, y;
			defaults = {
				size: 100,
				dialColor: '#ffffff',
				dialBackgroundColor: 'transparent',
				secondHandColor: '#FF3000',
			...
			function drawDial(color, bgcolor) {
				var dialRadius, dialBackRadius, i, ang, sang, cang, sx, sy, ex, ey, nx, ny, text, textSize, textWidth, brandtextWidth, brandtextWidth2;
				...
				for (i = 1; i <= 12; i += 1) {
					...
					cang = Math.cos(ang);
					if (i % 3 === 0) {
						ctx.lineWidth = parseInt(el.size / 50, 20);
						...
						textSize = parseInt(el.size / 18, 36);
			...
			function drawMinuteHand(minutes, color) {
				...
				ctx.save();
				ctx.lineWidth = parseInt(el.size / 50, 4);
				ctx.lineCap = "round";
				ctx.strokeStyle = color;
			...
			function drawHourHand(hours, color) {
				var hhlength = el.size / 3.2;
				ctx.save();
				ctx.lineWidth = parseInt(el.size / 20, 7);
				ctx.lineCap = "round";
				ctx.strokeStyle = color;
				ctx.rotate(toRadians(hours * 30));
				ctx.shadowColor = 'rgba(0,0,0,0)';
				ctx.shadowBlur = parseInt(el.size / 20, 7);
			...
			function startClock(x, y) {
				var theDate, s, m, hours, mins, h, exth, extm, allExtM, allAlarmM, atime;
				theDate = new Date();
